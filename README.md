...
## ToDo
*Split this list in to new features vs bugs.*
...
- transition to @Publish and use protocols for MIDI interfacing to allow for robust testing of the main session / Maschine Interface / router. whatever the class ends up being called. The ReactiveSwift → Combine migration, and the init observer-wiring cleanup that rides with it.
...
- Switch to Document undo manager, but this needs special care. there are notes at `Document.engineUndoManager`
- remove print() calls
...
## Feature Notes
redo and undo will affect locked cells. This is a design choice.